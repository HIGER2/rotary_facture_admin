...
definePageMeta({
  breadcrumb: 'navbar.bread.invoices_unpaid'
})
...
const handleListe =async (params:any={}) => {
   if (factures.value.length == 0) {
    ...
    const queryParams = new URLSearchParams({
        ...
        statusPayment:JSON.stringify(['en_attente','partiellement_payé'])
    });
    ...
    <NuxtLayout v-slot="{ userConnected }">
        <div class="w-full">
            <div class="w-full p-2">
                ...
                <div class="w-full">
                    <FactureTableComponent  type="invoice_unpaid" :columns="storeFacture.columns" :loading="loading" :user="userConnected" :data="factures"/>
                </div>
            </div>
        </div>
        </NuxtLayout>
